...
let
    ...
    for g = 0.1:0.1:4
        ...
        for j = 1:N
            os2 +=
                -1 / 2 * gamma^2, (transpose(S_minus_backward) * conj(S_minus_backward))', j
        end
        ...
        push!(
            avg_sx_array,
            real(inner(identity, apply(S_x_op, psi))) / real(inner(identity, psi)),
        )
        push!(
            avg_sy_array,
            real(inner(identity, apply(S_y_op, psi))) / real(inner(identity, psi)),
        )
        push!(
            avg_sz_array,
            real(inner(identity, apply(S_z_op, psi))) / real(inner(identity, psi)),
        )
    end
