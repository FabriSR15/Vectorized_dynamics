...
let 
    ...
    for j = 1:N-1
        ...
        os += -1/2 * gamma^2, transpose(S_minus_backward), j, conj(S_minus_backward), j+1
        os += -1/2 * gamma^2, transpose(S_minus_backward), j+1, conj(S_minus_backward), j
        os += -1/2 * gamma^2, transpose(S_minus_backward), j+1, conj(S_minus_backward), j+1
    end 
    ...
    for j = 1:N-1
        ...
        os2 += -1/2 * gamma^2, transpose(S_minus_backward)', j, conj(S_minus_backward)', j+1
        os2 += -1/2 * gamma^2, transpose(S_minus_backward)', j+1, conj(S_minus_backward)', j
        os2 += -1/2 * gamma^2, conj(S_minus_backward)', j+1, transpose(S_minus_backward)', j+1
    end 
